...
export const Back: React.FC<IProps> = props => {
  ...
  console.log('backback====',current,currentProduct);
  ...
  const doBack = () => {
    if (currentProduct !== 'index') {
      ...
      setTimeout(() => {
        // 重置0 // 返回首页
        dispatch(contextSlice.actions.setCurrentInit({ product:currentProduct }));
      }, 500);
    } else {
      // 第一页上翻
      dispatch(contextSlice.actions.changeCurrent({ product:currentProduct }));
    }
  };
